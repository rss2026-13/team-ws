...
class TrafficLightNode(Node):
    ...
    The distance check is re-enabled now that homography provides a live,
    accurate position rather than a fixed default.

    Parameters:
        stop_distance  (float): metres at which to stop (default 2.0 m)
    """
    ...
    def image_callback(self, image_msg: Image):
        """Run color segmentation to confirm whether the detected light is red."""
        try:
    ...
    def drive_input_callback(self, drive_msg: AckermannDriveStamped):
        """
        Intercept path planner commands.
        Drop the command while stopped (stop_timer handles publishing speed=0).
        Forward unchanged when not stopped.
        """
