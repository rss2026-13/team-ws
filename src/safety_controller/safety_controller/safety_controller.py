...
class SafetyController(Node):
    ...
    def evaluate_safety(self):
        ...
        if abs(delta) < 0.01: # Straight Path
            ...
            # Note: This assumes LIDAR is at (L - offset, 0) relative to rear axle
            # and CAR_LENGTH is the wheelbase.
            R = self.CAR_LENGTH / np.tan(delta)
            R_max = np.sqrt(self.CAR_LENGTH**2 + (abs(R) + self.CAR_WIDTH/2)**2)
            R_min = abs(R) - self.CAR_WIDTH/2

            px_cor = px + self.CAR_LENGTH - self.LIDAR_OFFSET
            py_cor = py - R
            ...
            bumper_angle = np.arcsin(np.clip(self.CAR_LENGTH / pr, -1.0, 1.0))
            p_bumper_ahead_dist = (pangle - bumper_angle) * pr 
            
            # Adjusting arc check: Obstacle must be past the bumper 
            # but within stopping distance
    ...
    def publish_safety_marker(self, delta, stop_dist):
        ...
        if abs(delta) < 0.01:
            ...
            theta_start_min = np.arcsin(np.clip(self.CAR_LENGTH / R_min, -1.0, 1.0))
            theta_start_max = np.arcsin(np.clip(self.CAR_LENGTH / R_max, -1.0, 1.0))
