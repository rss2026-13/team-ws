...
from launch.actions import GroupAction, Shutdown # Additional import
import signal
...
def generate_launch_description():
    ...
    ############################################################################
    ### Functionallity for bags
    ############################################################################
    recorded_topics = ["/distance", "/angle", "/scan", "/pose", "/drive", "/tf", "/tf_static"]

    def create_test_group(test_node, test_name):
    # 1. Define the recorder as a variable
        recorder = ExecuteProcess(
            cmd=['ros2', 'bag', 'record', '-o', f'bags/bag_{test_name}_1', *recorded_topics],
            output='screen',
        )
            ...
            actions=[
                ...
                RegisterEventHandler(
                    OnProcessExit(
                        ...
                        on_exit=[
                            LogInfo(msg=f"Finished {test_name}. Saving bag..."),
                            # 2. Add 2 seconds so the final stop is captured
                            TimerAction(
                                period=2.0,
                                actions=[
                                    # 3. Send SIGINT (Ctrl+C) ONLY to THIS recorder
                                    EmitEvent(event=SignalProcess(
                                        signal_number=signal.SIGINT, 
                                        process_matcher=lambda p: p == recorder
                                    ))
                                ]
                            )
                        ]
    ...
    test1_group = create_test_group(test1, "short_right_close")
    test2_group = create_test_group(test2, "short_left_far")
    test3_group = create_test_group(test3, "short_right_angled")
    test4_group = create_test_group(test4, "short_left_far_angled")
    test5_group = create_test_group(test5, "long_right")
    test6_group = create_test_group(test6, "long_left")
    ...
    return LaunchDescription([
        ...
        # FINAL EXIT: When Test 6 finishes, shut everything down
        RegisterEventHandler(
            OnProcessExit(
                target_action=test6,
                on_exit=[
                    LogInfo(msg='All tests complete. Shutting down and closing final bag.'),
                    Shutdown()
                ]
            )
        ),
