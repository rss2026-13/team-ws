...
class DynamicCamPublisher(Node):
    ...
    def __init__(self):
        ...
        #listener
        self.target_frame = 'left_cam'
        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)

        # self.target_frameOdom = 'odom'
        # self.tf_bufferOdom = Buffer()
        # self.tf_listenerOdom = TransformListener(self.tf_bufferOdom, self)

        #broadcaster
        self.tf_broadcasterBase = TransformBroadcaster(self)
    ...
    def timer_callback(self):
        try:
            ...
            leftCamT = self.toMatrix(t)
            # invert the L transform to go back to base
            baseCamT = np.linalg.inv(self.LTransform)
            odom_to_base = leftCamT @ baseCamT
            
            
            self.tf_broadcasterBase.sendTransform(self.toTransformStamped(t, odom_to_base, 'odom', 'base_link_2'))
            self.get_logger().info(f'Broadcasting BaseLink2!')
