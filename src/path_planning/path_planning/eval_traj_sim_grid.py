...
# ================= CONFIGURATION =================
EVAL_MODE = "GRID"  # "SAMPLING" or "GRID"

CONFIG = {
    "SAMPLING": {"internal_topic": "/trajectory/current"},
    "GRID":     {"internal_topic": "/trajectory/current"},
}
# =================================================
...
class LiveEvaluator(Node):
    def __init__(self):
        ...
        path_topic = CONFIG[EVAL_MODE]["internal_topic"]
        self.get_logger().info(f"Mode: {EVAL_MODE} | Listening on: {path_topic}")
    ...
    def print_summary(self):
        ...
        SEP = "=" * 58
        print(f"\n{SEP}")
        print(f"  Evaluation Summary — {EVAL_MODE}  ({len(self.runs)} goal(s))")
        print(SEP)
...
def plot_summary(runs: list[dict], mode: str, out_dir: str = "."):
    """Save a 2x2 figure with per-goal bars and a trend line for each metric."""
    if not runs:
        return
    ...
    #path = os.path.join(out_dir, f"eval_{mode}_{ts}.png")
...
def main(args=None):
    ...
    try:
        ...
        node.print_summary()
        plot_summary(node.runs, EVAL_MODE)
        node.destroy_node()
        rclpy.shutdown()
